...
const UserTable = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_NODE_BACKEND_URL}/auth/getuser`);
        if (res.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_NODE_BACKEND_URL}/auth/deleteuser/${id}`);
