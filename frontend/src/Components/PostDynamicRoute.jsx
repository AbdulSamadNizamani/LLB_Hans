...
const PostDynamicRoute = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_NODE_BACKEND_URL}/posts/idpost/${id}`);
        if (res?.status === 200) {
  ...
  const deletepost = async (id) => {
    try {
      setIsLoading(true);
      const res = await axios.delete(`${import.meta.env.VITE_NODE_BACKEND_URL}/posts/deletepost/${id}`);
      if (res?.status === 200) {
        setIsLoading(false);
