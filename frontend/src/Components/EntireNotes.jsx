...
const EntireNotes = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_NODE_BACKEND_URL}/notes/allnotes`);
        if (res?.status === 200) {
  ...
  const Delete = async (id) => {
    try {
      setIsLoading(true);
      const res = await axios.delete(`${import.meta.env.VITE_NODE_BACKEND_URL}/notes/deletenotes/${id}`);
      if (res?.status === 200) {
