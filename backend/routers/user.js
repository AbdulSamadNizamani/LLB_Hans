...
const cloudinary = require("cloudinary"); // Use `.v2` if required by your Cloudinary configuration
...
router.post("/login", async (req, res) => {
  try {
   ...
   res.cookie("token", token, {
      ...
      secure: true,
      sameSite: "None",
     domain: "https://llb-hans.vercel.app",
    });
    return res.status(200).json({
...
router.get('/Loggeduserdata', async (req, res) => {
  try {
    ...
    const decoded = jwt.verify(token, process.env.SECRET_KEY);
    const id = decoded.id;
    const user = await User.findById(id).select('-password').sort({
      createdAt: -1
    });
    if (!user) {
      return res.status(404).json({
...
router.patch('/addmanager/:id', async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: 'Added as Manager Successfully'
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
